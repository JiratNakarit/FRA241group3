    ...
    def __init__(self, parent, controller):
        ...
        label_1st = tk.Label(self,
                             text="\n\n\n\nThe first step of using vending machines, electronic devices allow users to select \n "
                                  ...
                                  "-----------------------------------------------------------------------------------------------\n"
                                  "The system website : http://hello world\n\n\n"
...
class PageTwo(tk.Frame):
    def __init__(self, parent, controller):
        ...
        # create title
        label2nd_title = tk.Label(self, text="This is your order", fg='SystemWindow', font='times 40 underline')
        label2nd_title.pack(side="top", fill="x", pady=0)
        ...
        for i in range(self.row):
            ...
            unit = Label(self, text="piece", font=BUTTON_FONT, bg='lavender')
            ...
            if i == self.row - 1:
                Notsure = tk.Button(self, text="Not sure", bg="salmon", font=BUTTON_FONT,
                                    command=lambda: controller.show_frame("PageOne"))
                Notsure.place(x=20, y=self.y)
                sure = tk.Button(self, text="sure", bg="OliveDrab2", font=BUTTON_FONT, command=self.Show_Product)
                sure.place(x=700, y=self.y)

        # Debug
        for i in range(self.row):
            print Name_ofIC[i].get(), quantity[i].get()
